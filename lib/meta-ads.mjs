...
const META_STATUS_MAX_LENGTH = 50;

function clipMetaStatusString(value) {
  if (value === undefined || value === null || value === '') return null;
  const text = String(value).trim();
  return text ? text.slice(0, META_STATUS_MAX_LENGTH) : null;
}

function normalizeMetaStatusField(value) {
  if (value === undefined || value === null || value === '') return null;
  if (Array.isArray(value)) {
    const parts = value.map((item) => normalizeMetaStatusField(item)).filter(Boolean);
    return clipMetaStatusString(parts.join(','));
  }
  if (typeof value === 'object') {
    if (value.code !== undefined && value.code !== null && value.code !== '') {
      return clipMetaStatusString(value.code);
    }
    if (value.description) {
      return clipMetaStatusString(value.description);
    }
    return null;
  }
  return clipMetaStatusString(value);
}
...
function resolveApproximateAudienceCount(audience) {
...
async function upsertMetaCustomAudience(audience, metaAccountId) {
  ...
  await pool.execute(
       ...
       raw_payload = VALUES(raw_payload)`,
    {
      ...
      approximateCount: resolveApproximateAudienceCount(audience),
      deliveryStatus: normalizeMetaStatusField(audience.delivery_status),
      operationStatus: normalizeMetaStatusField(audience.operation_status),
      syncedAt,
...
export async function syncMetaCustomAudiences({ skipAudit = false } = {}) {
  ...
  const fields = [
    ...
    'delivery_status', 'operation_status', 'data_source',
...
export async function listMetaAdSyncRuns({ limit = 20 } = {}) {
  ...
  const [rows] = await pool.execute(
    `SELECT * FROM meta_ad_sync_runs ORDER BY started_at DESC, id DESC LIMIT ${safeLimit}`
  );
  return rows;
}
...
export async function getMetaAdsReport({
  ...
  const params = toSqlParams({ insightLevel, startDate, endDate });
  if (accountId) {
  ...
  const campaignWhere = [];
  const campaignParams = toSqlParams({ startDate, endDate, insightLevel });
  if (accountId) {
  ...
  const adWhere = [];
  const adParams = toSqlParams({ startDate, endDate, insightLevel });
  if (accountId) {
